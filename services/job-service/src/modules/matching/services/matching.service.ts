...
export class MatchingService {
  ...
  async findMatchesForUser(userId: string, limit = 10): Promise<MatchedJob[]> {
    const profile = this.getProfile(userId);
    const allJobs = await this.aggregatorService.aggregateJobs();

    const scoredJobs = await Promise.all(
      allJobs.map(async (job) => {
        const matchResult = await this.scoringService.calculateMatchScore(profile, job);
  ...
  async scoreForPair(profile: UserProfile, jobId: string): Promise<MatchedJob> {
    const job = await this.jobsService.findById(jobId);
    if (!job) {
      throw new NotFoundException(`Job "${jobId}" not found`);
    }
  ...
  private generateHeuristicExplanation(
    ...
    const matchedSkills = job.skills.filter((s) => userSkillsLower.has(s.toLowerCase()));
    const missingSkills = job.skills.filter((s) => !userSkillsLower.has(s.toLowerCase()));
  ...
  private seedMockProfile(): void {
    this.mockProfiles.set('user_demo', {
      ...
      preferences: {
        ...
        remotePreference: RemotePreference.REMOTE,
        preferredCompanySize: [CompanySize.MEDIUM, CompanySize.LARGE],
