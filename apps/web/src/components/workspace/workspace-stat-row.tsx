...
function StatCard({ label, value, decimals = 0 }: StatProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'rounded-fw-card border border-border border-b-[3px] border-b-fw-orange bg-panel px-5 py-4 transition-shadow duration-200 hover:shadow-md',
      )}
    >
      <p className="font-mono text-3xl font-semibold tabular-nums text-foreground">{shown}</p>
