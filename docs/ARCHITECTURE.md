# CareerOS — Architecture Deep Dive

> This document provides a comprehensive look at the system architecture, design decisions, patterns, and tradeoffs behind CareerOS.
...
CareerOS is a **distributed microservices system** organized as a TypeScript monorepo. It comprises:
...
| **Type Safety End-to-End** | Shared TypeScript interfaces (`@career-os/shared-types`) ensure consistent contracts from frontend to backend |
...
The agent system is built on `@career-os/agent-core`, which provides:
...
Persistent volumes for Postgres and Redis data. All services on a shared `career-os-network` bridge.
...
| Resource | Purpose |
|----------|---------|
| `namespace.yaml` | Isolates CareerOS resources in `career-os` namespace |
...
| `ingress.yaml` | TLS-terminated routing at `api.careeros.dev` via cert-manager |
...
All services use **Pino** for structured JSON logging via `@career-os/utils`:
